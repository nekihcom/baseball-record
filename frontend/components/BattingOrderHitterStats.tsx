...
export function BattingOrderHitterStats({ stats }: Props) {
  const labels = [...Array(12)].map((_, i) => `${i + 1}` as const);
  const REST_LABEL = "それ以降";
  ...
  for (const row of source) {
    ...
    const key =
      order != null && order >= 1 && order <= 12 ? String(order) : REST_LABEL;
    const label = key;
  ...
  const pushRow = (key: string, label: string) => {
    ...
    rows.push({
      ...
      avg,
      scoringAvg,
      scoringAb: g.scoringAb,
      scoringHit: g.scoringHit,
    });
  };

  labels.forEach((label) => pushRow(label, label));
  pushRow(REST_LABEL, REST_LABEL);
