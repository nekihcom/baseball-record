...
type UsageKey = "先発" | "2番手" | "それ以降";

function getUsageKey(order: number | null): UsageKey {
  if (order == null) return "それ以降";
  if (order === 1) return "先発";
  if (order === 2) return "2番手";
  return "それ以降";
}

const USAGE_ORDER: UsageKey[] = ["先発", "2番手", "それ以降"];
...
export function UsagePitcherStats({ stats }: Props) {
  const source = Array.isArray(stats) ? stats : [];
  const groupMap = new Map<UsageKey, AggregatedRow>();
  ...
  for (const row of source) {
    const usage = getUsageKey(row.order ?? null);
    const g = groupMap.get(usage)!;
  ...
  const rows = USAGE_ORDER.map((key) => groupMap.get(key)!);
      ...
      <table className="min-w-max text-sm border-collapse">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (
              <tr key={row.usage} className="border-b last:border-b-0">
                <td className="px-2 py-1 whitespace-nowrap">{row.usage}</td>
